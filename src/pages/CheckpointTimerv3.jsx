...
export default function CheckpointTimer() {
  ...
  const captureLap = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      setMessage(isBibFirst ? 'Checkpoint saved locally, waiting to sync' : 'Tap saved locally, waiting to sync')
  ...
  const undoLastCheckpoint = useCallback(async () => {
    ...
    if (error) {
      ...
      savePendingLocal(eventId, checkpointId, pendingLocal)
      setMessage('Undo saved locally, waiting to sync')
    } else {
      setMessage('Last checkpoint undone')
    }

    setTimeout(() => setMessage(''), 1500)
  }, [laps, eventId, checkpointId])
