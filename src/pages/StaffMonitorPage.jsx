...
function StaffMonitorInner() {
  ...
  return (
    <div style={pageStyle}>
      <div style={cardStyle}>
        <div style={eyebrowStyle}>Staff Monitor</div>
        <h1 style={titleStyle}>{race?.name || session?.raceName || 'Race'}</h1>

        <div style={metaStyle}>
          <div><strong>Role:</strong> Monitor</div>
        ...
        {!loading && !error ? (
          <>
            <div style={noteStyle}>
              This is the limited monitor access shell. Next step is to connect this role to the race monitoring view only.
            </div>

            <div style={buttonRowStyle}>
              <button
                type="button"
                onClick={() => navigate(`/race/${session.raceEventId}/monitor`)}
...
export default function StaffMonitorPage() {
  return (
    <StaffRouteGuard allowedRole="monitor">
      <StaffMonitorInner />
    </StaffRouteGuard>
  )
}
