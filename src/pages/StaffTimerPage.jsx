...
function StaffTimerInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase
        .from('race_events')
        .select('id, name, status, race_started_at, race_finished_at')
  ...
  return (
    <div style={pageStyle}>
      <div style={cardStyle}>
        <div style={eyebrowStyle}>Staff Timer</div>
        <h1 style={titleStyle}>{race?.name || session?.raceName || 'Race'}</h1>

        <div style={metaStyle}>
          <div><strong>Role:</strong> Timer</div>
        ...
        {!loading && !error ? (
          <>
            <div style={noteStyle}>
              This is the limited timer access shell. Next step is to route this role into the timing-only capture flow.
            </div>

            <div style={buttonRowStyle}>
              <button
                type="button"
                onClick={() => navigate(`/race/${session.raceEventId}/checkpoints`)}
...
export default function StaffTimerPage() {
  return (
    <StaffRouteGuard allowedRole="timer">
      <StaffTimerInner />
    </StaffRouteGuard>
  )
}
