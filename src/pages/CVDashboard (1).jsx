...
export default function CVDashboard() {
  ...
  const [raceTime, setRaceTime] = useState('0:00')
  ...
  const startRace = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (elapsedRef.current > 15 && countRef.current < race.runners.length) {
        const add = Math.floor(Math.random() * 3) + 1
        countRef.current = Math.min(race.runners.length, countRef.current + add)
        setResults(generateResults(race.runners, countRef.current))
      }

      if (countRef.current >= race.runners.length && elapsedRef.current > 45) {
